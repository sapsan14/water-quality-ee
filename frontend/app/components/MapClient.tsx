...
function markerIcon(place: FrontendPlace) {
  ...
  const pulse = place.risk_level === "high" || place.official_compliant === 0;
  return L.divIcon({
    className: "",
  ...
  useEffect(() => {
    ...
    const group = markerClusterFactory({
      ...
      iconCreateFunction: (cluster: unknown) => {
        ...
        const size = count > 99 ? 52 : 46;
        return L.divIcon({
          html: `<div style="background:${color};color:#fff;border-radius:50%;width:${size}px;height:${size}px;display:flex;align-items:center;justify-content:center;border:3px solid rgba(255,255,255,0.95);font-weight:800;font-size:${count > 99 ? 13 : 15}px;font-family:system-ui,sans-serif;box-shadow:0 3px 12px rgba(0,0,0,0.4)">${count}</div>`,
          className: "",
          iconSize: [size, size],
    ...
    places.forEach((place) => {
      ...
      // On mobile: no popup — info shown in bottom sheet instead
