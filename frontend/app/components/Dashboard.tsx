...
/** ISO-like sample date → ms for sorting; null if missing/invalid. */
function sampleTimestampMs(place: FrontendPlace): number | null {
  const raw = place.sample_date;
  if (!raw) return null;
  const s = String(raw).trim();
  const iso = s.match(/^(\d{4}-\d{2}-\d{2})/);
  const t = iso ? Date.parse(`${iso[1]}T12:00:00Z`) : Date.parse(s);
  return Number.isNaN(t) ? null : t;
}

type PlacesTableSortKey = "date" | "prob" | "location" | "county";

function comparePlacesTableBase(a: FrontendPlace, b: FrontendPlace, key: PlacesTableSortKey, countyLabel: (c: string | null | undefined) => string): number {
  switch (key) {
    case "date": {
...
export default function Dashboard({ snapshot }: Props) {
  ...
  const watchlistPlaces = useMemo(() => {
    ...
    const list = snapshot.places.filter((p) => byId.has(p.id));
  ...
  return (
    <div className={`dashboard ${!isMobile ? (sidebarCollapsed ? "dashboardSidebarCollapsed" : "dashboardSidebar") : ""}`}>
          ...
          {watchlistPlaces.length === 0 ? (
            ...
            <ul className="alertList watchlistList">
              {watchlistPlaces.slice(0, 8).map((p) => (
                <li key={`watch-${p.id}`}>
                  <button className="linkBtn watchlistLocationBtn" type="button" onClick={() => selectPoint(p.id)}>
                    {p.location}
                  </button>
                  <span
        ...
        <div className="selectedPointHeader">
          ...
          {selectedPlace ? (
            <button
              ...
              onClick={() => toggleWatch(selectedPlace.id)}
              aria-pressed={watchlist.includes(selectedPlace.id)}
              aria-label={
                watchlist.includes(selectedPlace.id)
                  ? lruet(lang, "Убрать из избранного", "Eemalda jälgimisest", "Remove from watchlist")
                  : lruet(lang, "Добавить в избранное", "Lisa jälgimisse", "Add to watchlist")
              }
              title={
                watchlist.includes(selectedPlace.id)
                  ? lruet(lang, "Убрать из избранного", "Eemalda jälgimisest", "Remove from watchlist")
            ...
            >
              <span
                className="cellIcon"
                style={{ color: watchlist.includes(selectedPlace.id) ? "#f59e0b" : "var(--muted)" }}
              >
                <Icon name={watchlist.includes(selectedPlace.id) ? "star" : "star-outline"} />
              </span>
            </button>
          ) : null}
        </div>
          ...
          <div className="pointGrid">
            <div className="panel reportPanel">
              ...
              <div className="pointCardMeta">
                <span className="pointMetaTag">{selectedPlace.domain} / {selectedPlace.place_kind}</span>
                <span className="pointMetaTag">{countyPretty(selectedPlace.county || "Unknown")}</span>
              </div>
              <div className="pointCardRow">
                <span className="pointCardLabel">{lruet(lang, "Проба", "Proov", "Sample")}</span>
                <span>{fmtDate(selectedPlace.sample_date)}</span>
              </div>
              <div className="pointCardRow">
                <span className="pointCardLabel">{lruet(lang, "Статус", "Staatus", "Status")}</span>
              ...
              <div className="pointCardModels">
                {([
                  ["LR", selectedPlace.lr_violation_prob, lruet(lang, "Logistic Regression — линейная вероятностная модель", "Logistic Regression — lineaarne tõenäosusmudel", "Logistic Regression — linear probability model")],
                  ...
                  {!selectedPlace && clusterPlaces ? (
                    ...
                    <>
                      ...
                      <div className="gmWatchlistRow">
                        <button
                          type="button"
                          className={`gmWatchlistBtn ${watchlist.includes(selectedPlace.id) ? "gmWatchlistBtnOn" : ""}`}
                          onClick={() => toggleWatch(selectedPlace.id)}
                          aria-pressed={watchlist.includes(selectedPlace.id)}
                        >
                          <span className="cellIcon" style={{ color: watchlist.includes(selectedPlace.id) ? "#f59e0b" : "var(--muted)" }}>
                            <Icon name={watchlist.includes(selectedPlace.id) ? "star" : "star-outline"} />
                          </span>
                          {watchlist.includes(selectedPlace.id)
                      ...
                      {Object.keys(selectedPlace.measurements || {}).length ? (
                        <>
                          ...
                          <p className="hint gmMeasurementsHint">
                            {lruet(
                              lang,
                              "По алфавиту. Нажмите ячейку — справка или норма.",
                              "Tähestikuliselt. Klõpsa — info või norm.",
                              "A–Z. Tap a cell for info or norm."
                            )}
                          </p>
                          <div className="tableWrap compact">
                            <table className="table">
      ...
      <section className="panel">
        ...
        <p className="hint placesTableIntro">
          {lruet(
            lang,
            `Таблица показывает точки, которые проходят фильтры слева (до 250 строк из ${filtered.length}). Столбцы «Локация», «Уезд», «Дата пробы» и P(v) можно отсортировать — нажмите заголовок. По умолчанию: новые пробы выше.`,
            `Tabel näitab vasakpoolsete filtritega vastavaid punkte (kuni 250 rida ${filtered.length}-st). Sortimiseks klõpsa veerupealset. Vaikimisi: värskemad proovid üleval.`,
            `This table lists places that match the current filters (up to 250 of ${filtered.length}). Click a column header to sort. Default: newest sample first.`
          )}
        </p>
        <div className={`tableWrap ${isMobile ? "mobileResponsiveTable" : ""}`}>
          <table className="table placesTable">
              ...
              <tr>
                <th scope="col">
                  <button
                    ...
                    onClick={() => cyclePlacesTableSort("location")}
                    aria-pressed={placesTableSort.key === "location"}
                  >
                ...
                <th scope="col">
                  <button
                    type="button"
                    className="tableSortBtn"
                    onClick={() => cyclePlacesTableSort("county")}
                    aria-pressed={placesTableSort.key === "county"}
                  >
                ...
                <th className="iconCol pvColHead" scope="col">
                  <button
                    type="button"
                    className="tableSortBtn tableSortBtnPv"
                    onClick={() => cyclePlacesTableSort("prob")}
                    aria-pressed={placesTableSort.key === "prob"}
                    title={lruet(lang, "P(нарушения) — вероятность по ML", "P(rikkumine) — ML tõenäosus", "P(violation) — ML probability")}
                  >
            ...
            <tbody>
              {placesTableRows.slice(0, 250).map((p) => {
