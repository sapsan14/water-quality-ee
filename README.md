...
│
└── docs/
    ├── ml_framing.md     # что предсказывает модель и чего НЕ может
...
|---------|----------------|
| **Recall (класс 0)** | Главная метрика: доля реальных нарушений, не пропущенных моделью |
| Precision (класс 0) | Точность тревог: сколько из «нарушений» модели — действительно нарушения |
| ROC-AUC | Общая разделяющая способность при всех порогах |
| Confusion Matrix | Наглядность ошибок: FN (пропущенные нарушения) и FP (ложные тревоги) |
