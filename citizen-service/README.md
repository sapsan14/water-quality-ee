...
## Интерпретация цветовой шкалы модели

При раскраске «По прогнозу модели» используется градиент от зелёного до красного:

| Цвет | P(нарушение) | Интерпретация |
|------|-------------|---------------|
| Зелёный | < 0.3 | Модель оценивает риск как низкий |
| Жёлтый/оранжевый | 0.3–0.7 | Умеренный риск, требует внимания |
| Красный | > 0.7 | Высокий риск нарушения по модели |

Эта шкала — **вспомогательный индикатор**, а не замена официальной оценки.
