...
официальный compliant, опционально вероятность нарушения по RF, координаты.
...
Для полного снимка также нужен joblib (модель пишется в artifacts/citizen_model.joblib).
"""
...
def main() -> None:
    ...
    if args.map_only:
        print("[citizen] режим --map-only: без матрицы X и без RF (только meta для карты)")
        ...
        _timer_print("3) SimpleImputer(median) fit_transform на всей матрице X", t_run, last)
        ...
        clf.fit(X_imp, y)
        _timer_print(
            "4) RandomForestClassifier.fit (120 деревьев, все строки, n_jobs=-1)",
            t_run,
            last,
        )
    ...
    if not args.map_only:
        ...
        print(f"[citizen] модель: {ARTIFACTS / 'citizen_model.joblib'}")
        _timer_print("8) joblib.dump(imputer + clf) → citizen_model.joblib", t_run, last)
    else:
    ...
    model_note = (
        ...
        if not args.map_only
        else " Прогноз модели в этом снимке не включён; пересоберите без --map-only для слоя модели на карте."
    )
