...
**County:** opendata often leaves `maakond` empty. `load_domain` / `load_all` can fill `county` via `src/county_infer.py` (`infer_county=True`, optional OpenCage with `geocode_county=True` and `OPENCAGE_API_KEY`). For modeling without leakage, use `engineer_features` → split → `fit_county_mapping(train)` → `encode_categoricals(df, county_mapping=...)` (see notebook 03). `build_dataset(df)` still fits county on all rows for backward compatibility.
